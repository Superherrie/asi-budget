-- 009: Add Civils as a third subcontractor type, posting to Cost of Civils
-- (200400). electrical -> 200310, data -> 200300, civils -> 200400.

alter table budget_subcontractor_lines drop constraint if exists budget_subcontractor_lines_kind_check;
alter table budget_subcontractor_lines add constraint budget_subcontractor_lines_kind_check
  check (kind in ('electrical', 'data', 'civils'));

update budget_accounts set input_type = 'subcontractor' where code = '200400';
delete from budget_lines
where account_id in (select id from budget_accounts where code = '200400');
...
from (
  ...
  from budget_ho_allocations h
  union all
  -- subcontractors: electrical -> 200310, data -> 200300, civils -> 200400
