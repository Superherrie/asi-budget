-- Exclude CAP from the internal reallocation rule.
-- CAP is the central Capitec/Old Mutual holding cost centre, not an operating
-- branch, so its own Capitec/Old Mutual revenue should not be charged the 5%
-- CAP fee or the 2.5% Head Office fee. CAP still COLLECTS the 5% from the
-- operating branches; only its own revenue is exempted as a source.
   ...
   FROM ( SELECT budget_lines.cycle_id,
        ...
        UNION ALL
         SELECT rc.cycle_id,
          ...
          WHERE cu.internal_charge_pct <> 0::numeric AND rc.cost_centre_id <> ( SELECT budget_cost_centres.id FROM budget_cost_centres WHERE budget_cost_centres.code = 'CAP'::text)
        UNION ALL
         SELECT rc.cycle_id,
            ( SELECT budget_cost_centres.id
                   FROM budget_cost_centres
        ...
        UNION ALL
         SELECT rc.cycle_id,
           ...
           FROM budget_revenue_customer_lines rc
             JOIN budget_customers cu ON cu.id = rc.customer_id
          WHERE cu.ho_charge_pct <> 0::numeric AND rc.cost_centre_id <> ( SELECT budget_cost_centres.id FROM budget_cost_centres WHERE budget_cost_centres.code = 'CAP'::text)
        UNION ALL
         SELECT rc.cycle_id,
