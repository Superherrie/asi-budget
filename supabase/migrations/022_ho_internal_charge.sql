-- Additional internal charge: for customers flagged with the 5% CAP charge,
-- also allocate 2.5% of their revenue to Head Office (cc 000).
-- Posts to a new sales line "Internal Sales - Head Office" (310200): the branch
-- is charged -ho_charge_pct% and HO receives +ho_charge_pct%, netting to zero.

insert into budget_accounts (code, name, section, sort_order, input_type, budgetable)
select '310200','Internal Sales - Head Office','sales',40,'internal_sales',true
where not exists (select 1 from budget_accounts where code='310200');

alter table budget_customers add column if not exists ho_charge_pct numeric not null default 0;
update budget_customers set ho_charge_pct = 2.5 where internal_charge_pct <> 0;
...
create or replace view budget_statement_lines as
 SELECT cycle_id,
   ...
   FROM ( SELECT budget_lines.cycle_id,
            ...
            budget_lines.m12
           FROM budget_lines
          WHERE budget_lines.account_id <> (( SELECT budget_accounts.id
                   FROM budget_accounts
                  WHERE budget_accounts.code = '409000'::text))
        UNION ALL
         SELECT e.cycle_id,
            e.cost_centre_id,
            el.account_id,
        ...
        UNION ALL
         SELECT c.id AS cycle_id,
          ...
          WHERE a.account_id = (( SELECT budget_accounts.id
                   FROM budget_accounts
                  WHERE budget_accounts.code = '409000'::text))
        UNION ALL
         SELECT cx.cycle_id,
            ...
            ( SELECT budget_accounts.id
                   FROM budget_accounts
                  WHERE budget_accounts.code = '409000'::text) AS account_id,
                CASE
                    WHEN cx.idx <= 1 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 2 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 3 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 4 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 5 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 6 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 7 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 8 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 9 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 10 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 11 THEN cx.mo
                    ELSE 0::numeric
                END AS "case",
                CASE
                    WHEN cx.idx <= 12 THEN cx.mo
                    ELSE 0::numeric
                END AS "case"
           FROM ( SELECT cl.cycle_id,
                    ...
                    (- abs(cl.amount)) / (
                        CASE cl.category
                            WHEN 'Furniture and Fittings'::text THEN 5
                            WHEN 'Plant and Machinery'::text THEN 5
                            WHEN 'Computer Equipment'::text THEN 3
                            WHEN 'Software'::text THEN 3
                            WHEN 'Leasehold Improvements'::text THEN 3
                            WHEN 'Motor Vehicles'::text THEN 4
                            WHEN 'Tools'::text THEN 2
                            ELSE 5
        ...
        UNION ALL
         SELECT rc.cycle_id,
            rc.cost_centre_id,
            ( SELECT budget_accounts.id FROM budget_accounts WHERE budget_accounts.code = '310200'::text) AS account_id,
            (- rc.m1) * cu.ho_charge_pct / 100.0,
            (- rc.m2) * cu.ho_charge_pct / 100.0,
            (- rc.m3) * cu.ho_charge_pct / 100.0,
            (- rc.m4) * cu.ho_charge_pct / 100.0,
            (- rc.m5) * cu.ho_charge_pct / 100.0,
            (- rc.m6) * cu.ho_charge_pct / 100.0,
            (- rc.m7) * cu.ho_charge_pct / 100.0,
            (- rc.m8) * cu.ho_charge_pct / 100.0,
            (- rc.m9) * cu.ho_charge_pct / 100.0,
            (- rc.m10) * cu.ho_charge_pct / 100.0,
            (- rc.m11) * cu.ho_charge_pct / 100.0,
            (- rc.m12) * cu.ho_charge_pct / 100.0
           FROM budget_revenue_customer_lines rc
             JOIN budget_customers cu ON cu.id = rc.customer_id
          WHERE cu.ho_charge_pct <> 0::numeric
        UNION ALL
         SELECT rc.cycle_id,
            ( SELECT budget_cost_centres.id FROM budget_cost_centres WHERE budget_cost_centres.code = '000'::text) AS cost_centre_id,
            ( SELECT budget_accounts.id FROM budget_accounts WHERE budget_accounts.code = '310200'::text) AS account_id,
            rc.m1 * cu.ho_charge_pct / 100.0,
            rc.m2 * cu.ho_charge_pct / 100.0,
            rc.m3 * cu.ho_charge_pct / 100.0,
